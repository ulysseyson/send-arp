...
void sendARPPacket(pcap_t* handle, Mac& eth_dmac, Mac& eth_smac, Mac& arp_smac, Ip& arp_sip, Mac& arp_tmac, Ip& arp_tip, bool isRequest){
    // cout << "send arp packet to " << string(arp_tip) << " from " << string(arp_sip) << " with " << string(arp_smac) << " to " << string(arp_tmac) << "\n";
	EthArpPacket packet;
	...
	packet.arp_.sip_ = ntohl(arp_sip);
	packet.arp_.tmac_ = arp_tmac;
	packet.arp_.tip_ = ntohl(arp_tip);
...
void getSenderInfo(pcap_t* handle, Mac& sender_mac, Ip& sender_ip, Mac& attacker_mac, Ip& attacker_ip){
    ...
    // while(true){
        ...
        // cout << "sender mac: " << string(captured_packet->arp_.smac()) << "\n";
